...
def write_excel_sheets(filename,data):
...
def read_excel(filename="",sh="sheet",skip=[0,1,2,3]): 
   ...
   return data
def lookup_id(data,id='',column='Feature Task ID'):
...
def get_id_list(data):
    ...
    for i in id_list:
       if(not i):
            #Pass on Blanks.
            continue
       cur_data = get_base_suffix(i)
       new_d = create_final_entry(cur_data,last_data)
       odata.append(new_d['value'])
       last_data = cur_data
    return odata
def create_final_entry(cur_data={},last_data=None):
    data ={}
    match cur_data['type']:
        case 'base_range':
            # <base><x-y>
        ...
        case _:
            ...
            data          = cur_data
            logging.error("Unknown Test Case ID Field Value: " + str(cur_data) )
    return data
def get_base_suffix(data):
    odata ={}
    #Range <base><x> - <y>
   ...
   # logging.info("m_base_range: " + str(m_base_range))
...
def expand_range(data):
    ...
    if(re_match):
        ...
        tc.append(data)
    return tc
def create_tc_range(variations,tcid):
      tcid_out = tcid
      if ( variations > 1 ):
          # check for Range
...
def feature_row_format(id,featureData,row):
        # end of data
        ld = lookup_id(featureData,id)
        copy_row = row.copy()
        ...
        if(len(ld) == 1):
            ...
            logging.error("Error: Feature Raw Data Lookup:" + id + " Returned: " + str(ld))
...
def format_filename(inputfile,suffix='_flat'):
    ...
    outfile = filebase + "_" + str(now) + suffix + ".xlsx"
    logfile = filebase + "_" + str(now) + ".log"
...
def flatten(inputfile):
    ...
    rawData = read_excel(inputfile,'Diags Raw Data',[0,1,2,3])
    featureData = read_excel(inputfile,'Feature Raw Data',[0])
    outdata=[]
    row_cnt_list =[]
    test_run_dict={}
    cnt = 5
    for row in rawData:
        ...
        if m_break:
            logging.info("Exiting on Line: " + str(cnt) )
            break
#        if not row['Feature Task ID']:
#            logging.info("Skip Row No Feature Task ID: " + str(cnt) + "Feature Task ID : " + str(row['Feature Task ID']))
#            continue
        if not isinstance(row['Total Variations'], Number)  or row['Total Variations'] == 0  :
            # Skip the Row if no Variations
            logging.info("Skip Row No Variations Row: " + str(cnt) + " Total Variations: " + str(row['Total Variations']))
            continue

   

        if row['Test Case ID']==' ':
            logging.info("Skip Row No Test Case ID Row: " + str(cnt) + " Test Case ID: " + str(row['Test Case ID']) )
            continue

        
        logging.info("Row:"+str(cnt)+" Row[ Feature Task ID]:" + str(row['Feature Task ID']))
        feature_ids = get_id_list(str(row['Feature Task ID']))
        logging.info("Row:"+str(cnt)+" List of Feature ID's:" + str(feature_ids))
        # Check for end of rows.

           # Create a List of Test Case  ID's
        tcid=row['Test Case ID']
        logging.info("Row:" + str(cnt) + " Test Case ID: " + str(tcid) )

        testcase_ids=[]
        if(not pd.isna(tcid) ):
            testcase_ids = get_id_list(tcid)
        # Create Seperated List
        tc_list = list_to_sepperated_list(testcase_ids)

        logging.debug("Original Row[id]:" + str(feature_ids))
        ...
        for id in feature_ids:
            ...
            feature_row = feature_row_format(id,featureData,row)
            ...
            if( not feature_row['Total Variations'] == ' ' ):
               ...
               variations = 0
#            logging.info("Total Variations: " + str(variations))
 
            logging.info("Feature ID:" + str(id) +" Test Case ID's Split: " + str(testcase_ids) + " Total Variations:" + str(variations)  )
...
def list_to_sepperated_list(data=None,sep=','):
        sep_list = ''
        for ii in data: 
            if(sep_list):
...
def flatdir(args=None,outputdir='./outputdir'):
        # next paramer is the project
        if len(args) == 1:
            inputdir = args[0]
        else:
            logging.error("Wrong Arguments: " + str(args))
            return
        # loop through the Input directoryu looking for Excel Files
        if( not os.path.isdir(inputdir) ):
            logging.info('Directory not Found: ' + str(inputdir) )
            return 
        ...
        for file in os.listdir(inputdir):
            ...
            if( not os.path.isfile(full_file) ):
                logging.info('File Not Found: ' + str(full_file) )
                continue

            outfile = format_filename(file)

            logging.info('Input file: ' + str(full_file) + " Output File: " + outfile)
            flat_outdata,test_run_list = flatten(full_file)

            # Write the file _flat
            outfile = format_filename(file)
            full_outfile = outputdir + "/" + outfile 
            write_excel(full_outfile,flat_outdata)

            # Write test_run_list _test_runs
            outfile = format_filename(file,"_test_runs")
            full_outfile = outputdir + "/" + outfile  
            write_excel(full_outfile,test_run_list)
...
def combine_req_ip(args=[],outdata={}):
    if len(args) == 3:
        reqfile = args[0]        
        flatip = args[1]
        outfile = args[2]
    else:
        logging.info("Wrong number of Arguments for --combine: " + str(args))
    ...
    if( os.path.isdir(flatip) ):
        for file in os.listdir(flatip):
          ...
          outdata = combine_rq_ip_file(reqfile,flatip_file,outdata)
    else:
        # Single File Presented.
        outdata = combine_rq_ip_file(reqfile,flatip, outdata)

    # Setup Logger and Output File
    outfile = format_filename(outfile)
    write_excel_sheets(outfile, outdata)


def combine_rq_ip_file(reqfile=None,flatipfile=None,outdata={}):
    
    # Read Flat IP Tracker
    fip = read_excel(flatipfile,"Sheet1",skip=None)

    # Read Requirement
    req_df = read_excel_df(reqfile)
    req_sheets = req_df.keys()
    logging.info("Requirements Sheets: " + str(req_sheets) )
    # Combine.
...
#        logging.info("Sub_IP: " + str(sub_ip) )
        ...
        for sheet in modules[sub_ip]:
            ...
            for req_row in req:
                ...
                if ( filter_tst(req_row['Name'],str(feature_id), r'^(.*?)\s+-') ):
                    fip_row = normalize_data(fip_row)
                    outdata[sheet].append({**req_row,**fip_row})
                   # logging.info("Match: req_row: " + str(req_row['Name']) + "fip_row: " + str( round(fip_row['Feature Task ID'],3) ) )
    # {sheet1:[{},{},{}...], Sheet2:[{},{},{}...]}
    return outdata
def normalize_data(row=None,cnvt_array= [ {'% Code Done': 'PERCENT'}, {'% TP Done': 'PERCENT'}] ):
...
def filter_tst(extract_string=None,string=None,pattern=r'^.*?\s+(.*)$'):
    ...
    if m:
        if m[1] == string :       
#           logging.info("Matched: " + str(module) + " String: " + string)
           result=True
    return result

def  read_excel_df(filename,sheet=None):
   xls = pd.ExcelFile(filename)
   data = []
...
def get_ip(string=None,pattern='^.*?_(.*?)_'):
    data = None
...
def main(args):
    ...
    if args.flatfile:
        logging.info('Not Supported option: ' + '--flatfile' )
        pass
    if args.combine:
        data = combine_req_ip(args.combine)
    if args.cyc_suite_tr:
        data = cyc_suite_tr(args.cyc_suite_tr)
...
#sys.argv.append('Krackan1_CCX_Diagnostics_IP_Status_Tracker.xlsm')
...
input_url_filelist = 'input_list.xlsx'
inputfile = 'NV48_PMM_Diagnostics_IP_Status_Tracker.xlsm'


#data = parse_input_list(input_url_filelist)
site = 'https://myworktor.amd.com'
inputdir ="./inputdir"
outputdir ="./outputdir"
...
for file in os.listdir(inputdir):
    ...
    logging.info(full_file)
    logging.info(file)
    logging.info(outfile)
    outdata = flatten(full_file,outfile)
    full_outfile = outputdir + "/" + outfile 
    write_excel(full_outfile,outdata)
